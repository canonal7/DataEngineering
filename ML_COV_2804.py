...
X_train, X_test, y_train, y_test = train_test_split(
    df[predictors_cov], df[target], test_size=0.2, random_state=42
)
...
# %%
scaler = MinMaxScaler()
X_train_smote_cov[["Age", "lead_days"]] = scaler.fit_transform(
    X_train_smote_cov[["Age", "lead_days"]]
)
X_train_ros_cov[["Age", "lead_days"]] = scaler.fit_transform(
    X_train_ros_cov[["Age", "lead_days"]]
)
X_train_rus_cov[["Age", "lead_days"]] = scaler.fit_transform(
    X_train_rus_cov[["Age", "lead_days"]]
)
...
# %%
run_and_evaluate_random_forest_classifier(
    X_train_ros_cov, y_train_ros_cov, X_test, y_test
)

# %%
run_and_evaluate_random_forest_classifier(
    X_train_rus_cov, y_train_rus_cov, X_test, y_test
)
...
def print_classification_report(y_test, y_pred):
    ...
    target_names = ["show", "no-show"]
    print(
        "This is the classification report for the inital support vector machine model."
    )
    print(classification_report(y_test, y_pred, target_names=target_names, digits=3))
